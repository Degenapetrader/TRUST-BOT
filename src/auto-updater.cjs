...
autoUpdater.autoInstallOnAppQuit = false; // Disable auto-install on quit, we handle it manually
...
if (process.platform === 'win32') {
  ...
  autoUpdater.autoDownload = false; // Disable auto-download on Windows to prevent race conditions
}
...
function setupAutoUpdaterEvents() {
  ...
  autoUpdater.on('error', (err) => {
    ...
    if (process.platform === 'win32' && (
      ...
      // Mark update as downloaded and continue with the process
      if (updateInfo && updateAvailable) {
        console.log('✅ Bypassing Windows signing error, marking update as ready');
        updateDownloaded = true;
        updateDownloading = false;
        // Send ready-to-install status instead of error
        sendStatusToWindow('ready-to-install', updateInfo);
    ...
    if (errorMessage.includes('Code signature') || 
      ...
      if (updateAvailable && updateInfo) {
        updateDownloaded = true;
        updateDownloading = false;
        console.log('✅ Continuing with update despite signature error');
        // Send ready-to-install status
        sendStatusToWindow('ready-to-install', updateInfo);
        showUpdateNotification(updateInfo);
      }
    ...
    if (process.platform === 'win32' && (
      ...
      // Instead of showing error, mark update as ready if we have update info
      if (updateInfo && updateAvailable) {
        console.log('✅ Converting Windows signing error to ready-to-install state');
        updateDownloaded = true;
        updateDownloading = false;
        sendStatusToWindow('ready-to-install', updateInfo);
        showUpdateNotification(updateInfo);
  ...
  autoUpdater.on('update-downloaded', (info) => {
    ...
    updateDownloaded = true;
    updateDownloading = false; // Download completed
    updateInfo = info;
