...
export class BuyBot {
  ...
  async checkAndApproveInput(wallet, amount) {
    ...
    const amountWei = ethers.parseUnits(amount.toString(), inputDecimals);
  ...
  async calculateAmountOut(amountIn, slippage) {
    ...
    if (!this.tokenInfo.poolAddress) {
      ...
      return {
        ...
        minAmountOut: ethers.parseUnits((amountIn * 0.9).toString(), this.tokenInfo.decimals), // 90% for safety
    ...
    const amountInWei = ethers.parseUnits(amountIn.toString(), inputDecimals);
  ...
  async executeBuy(wallet, inputAmount, slippage) {
    try {
      ...
      const amountInWei = ethers.parseUnits(inputAmount.toString(), inputDecimals);
