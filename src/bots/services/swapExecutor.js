...
export class SwapExecutor {
  ...
  static async executeTwoStepBuy(wallet, currencyInfo, tokenInfo, currencyAmount, customGasPrice = null, tracker = null) {
    try {
      ...
      const currencyAmountWei = ethers.parseUnits(currencyAmount.toString(), currencyInfo.decimals);
      let step1Promise;
      ...
      console.log(`   Executing parallel transactions: Step 1 (nonce ${baseNonce}) & Step 2 (nonce ${baseNonce + 1})`);
      
      // Execute both transactions in parallel
      const [step1Tx, step2Tx] = await Promise.all(transactions);
  ...
  static async executeETHBuy(wallet, tokenInfo, ethAmount, customGasPrice = null, tracker = null) {
    try {
      ...
      const ethAmountWei = ethers.parseUnits(ethAmount.toString(), 18);
