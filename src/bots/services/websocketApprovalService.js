  ...
  async initialize() {
    ...
    if (config.wsUrlInfura && config.rpcUrlInfura) {
      try {
        ...
        log(`✅ WebSocket Approval Service: Infura provider initialized`);
      } catch (error) {
        log(`❌ Infura WebSocket approval service initialization failed: ${error.message}`);
      }
    }
    ...
    if (config.wsUrl && config.rpcUrl) {
      try {
        ...
        log(`✅ WebSocket Approval Service: Alchemy provider initialized`);
      } catch (error) {
        log(`❌ Alchemy WebSocket approval service initialization failed: ${error.message}`);
      }
    }
