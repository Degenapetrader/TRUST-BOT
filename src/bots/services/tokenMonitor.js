...
function getWebSocketProviders() {
  ...
  // PRIMARY: Infura WebSocket (if available)
  if (config.wsUrlInfura && config.rpcUrlInfura) {
    try {
      ...
      log(`✅ Infura WebSocket provider loaded for balance monitoring`);
    } catch (error) {
      log(`⚠️ Failed to create Infura WebSocket provider: ${error.message}`);
    }
  }
  
  // FALLBACK: Alchemy WebSocket (from wallets.json)
  if (config.wsUrl && config.rpcUrl) {
    try {
      ...
      log(`✅ Alchemy WebSocket provider loaded for balance monitoring`);
    } catch (error) {
